...
static void fill_block(uint32_t *blk, byte *key, uint64_t counter, uint64_t nonce) {
    memcpy(blk, key_quarter_constant, strlen(key_quarter_constant));
    memcpy(blk + 4, key, 32);
    memcpy(blk + 12, &counter, 8);
    memcpy(blk + 14, &nonce, 8);
}

void chacha20_block(uint32_t *blk, byte *key, uint64_t counter, uint64_t nonce) {
